...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const isAdminRole = currentRole === "SUPER_ADMIN" || currentRole === "ADMIN" || currentRole === "PLATFORM_OWNER";
              ...
              {branches.length > 0 && (
                <div className="lg:hidden flex-1 min-w-0">
                  <Select value={selectedBranchId || "__all__"} onValueChange={(v) => setSelectedBranchId(v === "__all__" ? "" : v)}>
                    <SelectTrigger className="h-8 rounded-lg text-[11px] bg-primary/5 border-primary/10 text-primary font-medium px-2.5 gap-1 w-full">
