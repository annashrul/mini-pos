...
export function ClosingReportsContent() {
  ...
  const canReclosing = canAction("reclosing") && canPlan("closing-reports", "reclosing");
  const canExport = canAction("export") && canPlan("closing-reports", "export");

  const [detailOpen, setDetailOpen] = useState(false);
  const [report, setReport] = useState<ClosingReportData>(null);
  const [reportLoading, setReportLoading] = useState(false);

  const [detailAllowReopen, setDetailAllowReopen] = useState(false);
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="hidden sm:block">
          <ExportMenu module="closing-reports" branchId={selectedBranchId || undefined} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Desktop: DateRangePicker inline */}
        <div className="hidden sm:block">
          <DateRangePicker
            from={dateFrom}
            to={dateTo}
        ...
        <Button
          variant="outline"
          size="icon"
          className="sm:hidden relative h-11 w-11 rounded-xl shrink-0 border-border/40 bg-white shadow-sm"
          onClick={() => { setTempDateFrom(dateFrom); setTempDateTo(dateTo); setFilterSheetOpen(true); }}
        >
          <SlidersHorizontal className="w-4 h-4" />
          {(dateFrom || dateTo) && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center font-bold">1</span>
          )}
        </Button>
        {/* Mobile: Filter Sheet */}
        <Sheet open={filterSheetOpen} onOpenChange={setFilterSheetOpen}>
          <SheetContent side="bottom" className="rounded-t-2xl px-0 pb-0">
            <SheetHeader className="px-4 pb-3 border-b border-border/30">
              <SheetTitle className="text-sm font-semibold">Filter</SheetTitle>
            </SheetHeader>
            <div className="px-4 py-4 space-y-3">
              <div className="space-y-1.5">
                ...
                <DateRangePicker
                  from={tempDateFrom}
                  to={tempDateTo}
                  onChange={(f, t) => { setTempDateFrom(f); setTempDateTo(t); }}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center gap-3 min-w-0 shrink-0">
                    ...
                    <div className="flex items-center gap-0.5 sm:hidden">
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-teal-50 hover:text-teal-600 transition-colors" onClick={() => openDetail(row.id, !!row.allowReopen)} title="Lihat Detail">
                        <Eye className="w-4 h-4" />
                      </Button>
                      <DisabledActionTooltip disabled={!canReclosing || !!row.allowReopen} message={row.allowReopen ? "Sudah di-reclosing" : cannotMessage("reclosing")} menuKey="closing-reports" actionKey="reclosing">
                        <Button disabled={!canReclosing || !!row.allowReopen} variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-orange-500 hover:text-orange-700 hover:bg-orange-50 transition-colors"
            ...
            <>
              ...
              <DialogFooter className="px-4 sm:px-6 pb-4 sm:pb-6 shrink-0">
                <div className="flex flex-col-reverse sm:flex-row items-center justify-between w-full gap-2">
                  <DisabledActionTooltip disabled={!canReclosing || detailAllowReopen} message={detailAllowReopen ? "Sudah di-reclosing" : cannotMessage("reclosing")} menuKey="closing-reports" actionKey="reclosing">
                    <Button
                      disabled={!canReclosing || detailAllowReopen}
                      variant="outline"
                  ...
                  <DisabledActionTooltip disabled={!canExport} message={cannotMessage("export")} menuKey="closing-reports" actionKey="export">
                    <Button
                      disabled={!canExport}
                      variant="outline"
        ...
        <DialogContent className="rounded-2xl max-w-[calc(100vw-2rem)] sm:max-w-sm">
          ...
          <div className={`space-y-4 mt-1 ${!canReclosing ? "pointer-events-none opacity-70" : ""}`}>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <DisabledActionTooltip disabled={!canReclosing} message={cannotMessage("reclosing")} menuKey="closing-reports" actionKey="reclosing">
