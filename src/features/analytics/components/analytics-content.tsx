...
function ChartTooltipContent({ active, payload, label, formatValue }: { active?: boolean; payload?: ReadonlyArray<{ value?: unknown; name?: unknown; color?: string | number }>; label?: unknown; formatValue?: (value: number, name: string) => [string, string] }) {
...
function AnalyticsContentInner() {
    ...
    const activeTab = searchParams.get("tab") || "margin";

    const [loading, setLoading] = useState(true);
    const [loadedTabs, setLoadedTabs] = useState<Set<string>>(new Set());
    const [marginData, setMarginData] = useState<AnalyticsDataset["marginData"]>([]);
    const [categoryMargins, setCategoryMargins] = useState<AnalyticsDataset["categoryMargins"]>([]);
    const [deadStock, setDeadStock] = useState<AnalyticsDataset["deadStock"]>([]);
    const [slowMoving, setSlowMoving] = useState<AnalyticsDataset["slowMoving"]>([]);
    const [peakHours, setPeakHours] = useState<AnalyticsDataset["peakHours"]>([]);
    const [voidAbuse, setVoidAbuse] = useState<AnalyticsDataset["voidAbuse"]>([]);
    const [cashierPerf, setCashierPerf] = useState<AnalyticsDataset["cashierPerf"]>([]);
    const [dailyProfit, setDailyProfit] = useState<AnalyticsDataset["dailyProfit"]>([]);
    const [shiftProfit, setShiftProfit] = useState<AnalyticsDataset["shiftProfit"]>([]);
    const [supplierRanking, setSupplierRanking] = useState<AnalyticsDataset["supplierRanking"]>([]);
    const [supplierDebt, setSupplierDebt] = useState<AnalyticsDataset["supplierDebt"]>([]);
    const [unusualDiscounts, setUnusualDiscounts] = useState<AnalyticsDataset["unusualDiscounts"]>([]);
    const [promoEffectiveness, setPromoEffectiveness] = useState<AnalyticsDataset["promoEffectiveness"]>([]);
    const [reorderRecommendations, setReorderRecommendations] = useState<AnalyticsDataset["reorderRecommendations"]>([]);
    ...
    useEffect(() => {
        ...
        if (branchChanged) {
            ...
            setMarginData([]); setCategoryMargins([]); setDeadStock([]); setSlowMoving([]);
            setPeakHours([]); setVoidAbuse([]); setCashierPerf([]); setDailyProfit([]);
            setShiftProfit([]); setSupplierRanking([]); setSupplierDebt([]);
            setUnusualDiscounts([]); setPromoEffectiveness([]); setReorderRecommendations([]);
            setLoadedTabs(new Set());
        }
        loadTabData(activeTab, selectedBranchId || undefined);
    }, [branchReady, selectedBranchId, activeTab, loadTabData]);
