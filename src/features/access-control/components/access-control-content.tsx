...
import {
  ...
  Lock, Loader2, Plus, Pencil, Trash2, SlidersHorizontal, Check, MoreVertical,
} from "lucide-react";
              ...
              {roleKeys.map((roleKey) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className="absolute top-2 right-2 lg:hidden">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button className="h-6 w-6 rounded-lg flex items-center justify-center hover:bg-muted">
                          ...
                          <DropdownMenuContent align="end" className="rounded-xl w-40">
                            <DisabledActionTooltip disabled={!canUpdateRole} message={cannotMessage("update_role")} menuKey="access-control" actionKey="update_role">
                              <DropdownMenuItem disabled={!canUpdateRole} onClick={() => openEditRole(appRole)} className="text-xs gap-2">
                            ...
                            {!appRole.isSystem && (
                              <DisabledActionTooltip disabled={!canDeleteRole} message={cannotMessage("delete_role")} menuKey="access-control" actionKey="delete_role">
                                <DropdownMenuItem disabled={!canDeleteRole} onClick={() => setConfirmDelete(appRole)} className="text-xs gap-2 text-red-600 focus:text-red-600">
                    ...
                    {appRole && (
                      <div className="hidden lg:flex gap-1 px-3 pb-2 pl-[52px] opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <DisabledActionTooltip disabled={!canUpdateRole} message={cannotMessage("update_role")} menuKey="access-control" actionKey="update_role">
            ...
            <div className="flex items-center gap-2">
              <SearchInput
                ...
                onChange={(v) => {
                  ...
                  debounceRef.current = setTimeout(() => { fetchMatrix(v); }, 300);
                }}
