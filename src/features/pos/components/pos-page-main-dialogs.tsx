                            ...
                            <div className="space-y-2">{historyData.map((tx) => (
                                <div key={tx.id} className="rounded-xl border border-border/40 hover:bg-muted/20 transition-colors overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-1 px-2 pb-2 sm:px-3 sm:pb-2.5">
                                        <ProButton menuKey="pos" actionKey="reprint" onClick={() => reprintReceipt(tx)} className="h-7 rounded-lg text-[11px] gap-1 text-muted-foreground inline-flex items-center px-2 hover:bg-accent"><Printer className="w-3 h-3" /><span className="hidden sm:inline">Cetak</span></ProButton>
                                        ...
                                        {tx.status === "COMPLETED" && (
                                            <ProButton menuKey="pos" actionKey="void" onClick={() => { setVoidingId(tx.id); setVoidReason(""); setShowVoidDialog(true); }} className="h-7 rounded-lg text-[11px] gap-1 text-red-500 hover:bg-red-50 ml-auto inline-flex items-center px-2"><Ban className="w-3 h-3" /><span className="hidden sm:inline">Void</span></ProButton>
                                        )}
                                    </div>
                                </div>
                            ))}</div>
                    ...
                    {historyDetail && (
                        <DialogFooter className="px-4 pb-4 sm:px-6 sm:pb-6 pt-0 border-t border-border/30">
                            <div className="flex gap-2 w-full pt-3">
                                <ProButton menuKey="pos" actionKey="reprint" onClick={() => reprintReceipt(historyDetail)} className="flex-1 rounded-lg h-9 sm:h-10 text-xs sm:text-sm border inline-flex items-center justify-center gap-1.5 hover:bg-accent"><Printer className="w-3.5 h-3.5" /> Cetak Ulang</ProButton>
                                {historyDetail.status === "COMPLETED" && <ProButton menuKey="pos" actionKey="void" onClick={() => { setVoidingId(historyDetail.id); setVoidReason(""); setShowVoidDialog(true); }} className="flex-1 rounded-lg h-9 sm:h-10 text-xs sm:text-sm text-red-500 border border-red-200 hover:bg-red-50 inline-flex items-center justify-center gap-1.5"><Ban className="w-3.5 h-3.5" /> Void</ProButton>}
                            </div>
                        </DialogFooter>
                    )}
                </DialogContent>
                        ...
                        <div className="order-1 md:order-2 flex flex-col min-h-0 flex-1">
                            <ScrollArea className="flex-1 min-h-0 overflow-hidden p-4 md:p-5">
                              <div className="space-y-3 md:space-y-4">
                                ...
                                {canPosAction("split_bill") && <div className="space-y-1.5 rounded-lg border border-border/40 bg-muted/10 p-2.5 md:p-3">
